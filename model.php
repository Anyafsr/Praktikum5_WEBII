...
function tampilbuku()
{
    ...
    if(!empty($result)){
        foreach ($result as $buku) {
            ...
            echo "<td>"."<a href = 'formbuku.php?id_buku=".$buku['id_buku']."'>Edit Buku  </a><br><br>";
...
function tambahpeminjaman($id, $nama_member, $judul_buku, $tglpinjam, $tglbalik)
{
    $sql = "INSERT INTO peminjaman (id_peminjaman, nama_member, judul_buku, tgl_peminjaman, tgl_kembali) VALUES (:id_p, :nama_member, :judul_buku, :tgl_pinjam, :tgl_balik)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':id_p' => $id, ':nama_member' => $nama_member, ':judul_buku' => $judul_buku, ':tgl_pinjam' => $tglpinjam, ':tgl_balik' => $tglbalik));
    if (!empty($result)) {
...
function updatepeminjaman($id, $nama_member, $judul_buku, $tglpinjam, $tglbalik)
{
    $pdo_statement = koneksi()->prepare(
        "UPDATE peminjaman SET nama_member='" . $nama_member . "', judul_buku='" . $judul_buku . "', tgl_peminjaman='" . $tglpinjam . "', tgl_kembali='" . $tglbalik . "' WHERE id_peminjaman=" . $id
    );
    $result = $pdo_statement->execute();
    if($result){
        header('location:peminjaman.php');
